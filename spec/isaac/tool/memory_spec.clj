...
(describe "Memory tools"
  ...
  (it "writes to today's UTC note"
    (binding [sut/*now* (java.time.Instant/parse "2026-04-21T10:00:00Z")]
      (sut/memory-write-tool {:content "Hieronymus hates artichokes." :state-dir test-dir})
      (should= "Hieronymus hates artichokes."
               (fs/slurp (str test-dir "/crew/main/memory/2026-04-21.md")))))

  (it "accepts a vector of entries"
    (binding [sut/*now* (java.time.Instant/parse "2026-04-21T10:00:00Z")]
      (sut/memory-write-tool {:content ["Orpheus" "Grandma"] :state-dir test-dir})
      (should= "Orpheus\nGrandma"
               (fs/slurp (str test-dir "/crew/main/memory/2026-04-21.md")))))

  (it "appends instead of overwriting"
    (binding [sut/*now* (java.time.Instant/parse "2026-04-21T10:00:00Z")]
      (sut/memory-write-tool {:content "first" :state-dir test-dir})
      (sut/memory-write-tool {:content "second" :state-dir test-dir})
      (should= "first\nsecond"
               (fs/slurp (str test-dir "/crew/main/memory/2026-04-21.md")))))
  ...
  (it "reads notes across an inclusive date range"
    ...
    (let [result (sut/memory-get-tool {:start_time "2026-04-14" :end_time "2026-04-16" :state-dir test-dir})]
  ...
  (it "searches all memory files and returns ripgrep-style lines"
    ...
    (let [result (sut/memory-search-tool {:query "Orpheus" :state-dir test-dir})]
  ...
  (it "uses the session crew when provided"
    ...
    (binding [sut/*now* (java.time.Instant/parse "2026-04-21T10:00:00Z")]
      (sut/memory-write-tool {:content "tea note" :session-key "crew-session" :state-dir test-dir})
      (should= "tea note"
