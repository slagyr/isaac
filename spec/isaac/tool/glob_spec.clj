...
(describe "Glob tool"
  ...
  (it "returns matching file paths"
    ...
    (let [result (sut/glob-tool {"pattern" "**/*.clj" "state_dir" support/test-dir})]
  ...
  (it "sorts results by modification time descending with alphabetical tiebreaker"
    ...
    (let [result (sut/glob-tool {"pattern" "src/*.clj" "state_dir" support/test-dir})]
      (should-be-nil (:isError result))
  ...
  (it "defaults the search path to the session cwd"
    (let [state-dir   support/test-dir
          cwd         (str support/test-dir "/workspace")
