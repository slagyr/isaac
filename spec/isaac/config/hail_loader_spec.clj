...
(describe "config loader hail files"
  ...
  (it "loads hail band files into config"
    (let [mem  (fs/mem-fs)
      ...
      (fs/mkdirs mem (str state-dir "/config/hail"))
      (fs/spit mem (str state-dir "/config/hail/bean.ready.edn")
               "{:crew-tags [:role/worker] :reach :one}")
      (nexus/-with-nested-nexus {:fs mem}
        (let [result (sut/load-config-result {:state-dir state-dir :fs mem})]
  ...
  (it "loads hail markdown companions as prompts"
    (let [mem  (fs/mem-fs)
      ...
      (fs/spit mem (str state-dir "/config/hail/bean.ready.edn")
       ...
       (fs/spit mem (str state-dir "/config/hail/bean.ready.md")
                "Pick up the bean.")
       (nexus/-with-nested-nexus {:fs mem}
         (let [result (sut/load-config-result {:state-dir state-dir :fs mem})]
  ...
  (it "rejects hail bands without any addressing fields"
    (let [mem  (fs/mem-fs)
          state-dir "/tmp/isaac-home/.isaac"]
      (fs/mkdirs mem (str state-dir "/config/hail"))
      (fs/spit mem (str state-dir "/config/hail/empty.edn")
               "{}")
      (nexus/-with-nested-nexus {:fs mem}
        (let [result (sut/load-config-result {:state-dir state-dir :fs mem})]
