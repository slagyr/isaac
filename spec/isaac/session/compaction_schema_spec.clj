...
(describe "Compaction schema"
  ...
  (it "conforms a valid compaction config"
    (should= {:strategy :slinky :threshold 160.0 :head 80.0 :async? false}
  ...
  (it "returns readable field errors for negative values"
    (let [result (schema/conform sut/config-schema {:threshold -1 :head 80})]
      (should (schema/error? result))
      (should= {:threshold "must be a non-negative token count"}
               (schema/message-map result))))
  ...
  (it "accepts threshold values greater than 1"
    (should= {:threshold 160.0 :head 80.0}
             (schema/conform! sut/config-schema {:threshold 160 :head 80}))))
