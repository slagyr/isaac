...
(describe "Session Compaction"
  ...
  (describe "default-threshold"
    (it "uses the larger of 80% of the window or window minus frontmatter budget"
  ...
  (describe "default-head"
    (it "returns 30% of the window as a token count"
  ...
  (describe "resolve-config"
    (it "defaults to rubberband with absolute threshold and head token counts"
      (should= {:async? false :strategy :rubberband :head 9830.0 :threshold 26214.0}
               (sut/resolve-config {} 32768)))

    (it "merges session overrides"
      (should= {:async? false :strategy :slinky :head 80.0 :threshold 160.0}
  ...
  (describe "should-compact?"
    (it "uses the configured threshold token count"
    ...
    (it "for rubberband compacts the whole effective history"
      (let [entries [{:id "m1" :tokens 40}
        ...
        (should= {:compact-count 4 :first-kept-entry-id nil :tokens-before 170}
                 (sut/compaction-target entries {:strategy :rubberband :head 80 :threshold 160} 200))))
    ...
    (it "for slinky compacts only enough oldest entries to preserve the head"
      (let [entries [{:id "m1" :tokens 40}
        ...
        (should= {:compact-count 2 :first-kept-entry-id "m3" :tokens-before 80}
                 (sut/compaction-target entries {:strategy :slinky :head 80 :threshold 160} 200)))))
