...
(defn- write-index! [state-dir store]
...
(defn- read-session-store [state-dir]
  (let [path  (index-path state-dir)
        store (if (fs/exists? path)
                (normalize-index-store (edn/read-string (fs/slurp path)))
                (let [sidecars (read-sidecar-store state-dir)]
                  (when (seq sidecars)
                    (write-index! state-dir sidecars))
                  sidecars))]
  ...
  (let [store (read-session-store state-dir)]
    (when-let [id (resolve-entry-id store identifier)]
      (let [entry     (conform-session! (updater (get store id)))
        ...
        (write-index! state-dir new-store)
        entry))))
   ...
   (let [opts      (entry-defaults opts)
     ...
     (cond
       ...
       (let [session-file  (str id ".jsonl")
         ...
         (write-index! state-dir (assoc store id (conform-session! entry)))
