...
(declare get-session normalize-index-store session-id sessions-dir with-session-defaults)
...
(defn- read-sidecar-entry [state-dir file-name fs]
  (let [sid   (sidecar-session-id file-name)
    ...
    [sid (with-session-defaults (assoc entry :id sid))]))
...
(defn- write-index! [state-dir store fs]
  (let [path (index-path state-dir)]
    (mkdirs*! fs (fs/parent path))
    (spit*! fs path (write-edn store))))

(defn- read-session-store [state-dir fs]
  (let [path  (index-path state-dir)
        store (if (exists?* fs path)
                (normalize-index-store (edn/read-string (slurp* fs path)))
...
(defn- resolve-entry-id [store identifier]
  (cond
    ...
    :else (let [id (session-id identifier)]
            (when (contains? store id) id))))
   ...
   (let [opts               (entry-defaults opts)
     ...
     (cond
       ...
       (let [session-file  (str id ".jsonl")
         ...
         (write-index! state-dir (assoc store id (conform-session! entry)) fs)
...
(defn append-error!
  ...
  ([state-dir identifier error-entry fs]
   (let [entry            (get-session state-dir identifier fs)
     ...
     (append-entry! state-dir (:session-file entry) transcript-entry fs)
     (update-index-entry! state-dir identifier #(assoc % :updated-at now) fs)
     transcript-entry)))
