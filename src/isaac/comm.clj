...
(defprotocol Comm
  ...
  (on-compaction-start [ch session-key payload])
  (on-compaction-success [ch session-key payload])
  (on-compaction-failure [ch session-key payload])
  (on-compaction-disabled [ch session-key payload])
