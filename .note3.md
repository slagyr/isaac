...
**Next — not started**
10. Wire `door/register-trust-rules!` into `isaac.google.component/start!`
    (`src/isaac/google/component.clj:17`) so a tenanted host gets its per-tenant rules at boot; keep
    the manifest's single `:isaac.http/identity {:google-pubsub ...}` rule as the flat/default case.
...
12. Scenarios: new `features/tenants.feature` (the bean's four); `push_door.feature` and
    `login.feature` must pass unchanged. `bb ci` has not been run yet this bean — only `bb spec`.
