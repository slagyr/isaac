...
Done: Foundation retired-berth diagnostic is green/pushed (`bean/isaac-oc3f@adbeace`). Server runtime install/reloader teardown is a component and `isaac.server.app` now plainly delegates lifecycle to Foundation (`bean/isaac-oc3f@c60f1f7`). Episodes implements the component protocol with its full suite green/pushed (`bean/isaac-oc3f@ec2fc54`). Discord manifest cutover is pushed and its implementation/spec migration from the removed service API is in progress (`bean/isaac-oc3f@63d5dc0`).

Next: fix Discord startup ordering so a comm registered by the later server-runtime component connects while the Discord component is already active, then add production CLI-path acceptance, run all four repo suites, rebase, and record coordinated release/pin coordinates. Current RED: Discord JVM suite has 3 lifecycle failures because Discord starts before server-runtime installs config and `runner/running?` remains false until all components finish. Resume at `isaac-discord-6ele/src/isaac/comm/discord/service.clj:135` and rerun `bb jvm-spec spec/isaac/comm/discord/service_spec.clj`.
