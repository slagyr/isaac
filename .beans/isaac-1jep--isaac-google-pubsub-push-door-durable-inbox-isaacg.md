...
- Route `POST /google/pubsub` via `:isaac.server/route`. Verify the Pub/Sub push OIDC bearer: signature against Google's certs, issuer, `aud` = the configured door URL, `email` = the configured push service account. Anything else → 401/403, logged once per minute per reason (no log floods from the public internet).
...
2. Bad audience / wrong service account / bad signature → 403, nothing persisted.
