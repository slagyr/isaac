...
Done: agent branch `bean/isaac-g71i` remains pushed at `6499ae5` on base `origin/main@3e6bf39`; complete unit specs are green (1606 examples, 3304 assertions), tool-loop and parallel-tool fixtures are repaired, and the required wire-key grep is empty. Claude Code remains pushed at `fda900b` on base `origin/main@d26d410`, pins agent `6499ae5`, and `bb ci` is green (66 unit examples plus 43 feature examples; only 3 opt-in real specs pending).

RED: the focused agent context-overflow acceptance still fails after lowering the fixture below the pre-turn compaction threshold (`last-input-tokens` 100): the retry provider response is compaction-framed as `[Compacted history; not a new request] ... Pending request` instead of exact `Sails trimmed.`. Continue by tracing the overflow compaction request classification and queue consumption from `src/isaac/drive/turn.clj:1040` through `src/isaac/llm/api/grover.clj:250`. Exact resume command: `clojure -M:features features/llm/api/response_schema.feature:268`.
