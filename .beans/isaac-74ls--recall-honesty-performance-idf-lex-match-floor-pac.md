...
Follow-up to isaac-j2p4 from 2026-08-19 field trials (see isaac-51xy field notes). Honesty: IDF-weighted lexical channel with matched-term receipts; statistical match floor so junk queries say so. Performance: packed float32 vector store (metadata + vectors.bin), pre-normalized vectors, compiled-core scoring. Draft — planning session in progress.
