...
Done: implementation branch rebased onto `origin/main@a6c27f8`; head `fa461ff`. Target queue feature is green 5/5 before and after fixes (5 examples, 0 failures, 20 assertions each). Grover reset now releases every wait gate, scripted dequeue is atomic, turn-worker wake requests are coalesced instead of dropped, scenario teardown cancels uncooperative futures after bounded cooperative cleanup, and file assertions await async compaction. `compaction_memory_flush.feature` is green 5/5 after reproducing isolated red. `bb spec` is green sequentially: 1601 examples, 0 failures, 3302 assertions. Full JVM features produced three green runs (753 examples, 0 failures, 1790 assertions, 1 existing pending), but the latest five-run sequence reset at run 3 due the known CLI cancel flake.

Current red: full JVM run `/tmp/x4mr-jvm-clean-3.out` failed `features/session/cli.feature:367`; cancellation completed between the marker assertion and the fixture's in-flight assertion. A separate full run intermittently failed `cancel_aborts_work.feature:27`, where the 50ms tool completed before the test could cancel. `bb features` itself reaches its hard-coded 180s timeout on this machine despite unwrapped JVM runs finishing around 90s; native full-suite execution under concurrent load is slower.

Next: drive deterministic cancellation timing at the feature fixture rather than racing 50ms work or asserting an already-cancelled turn remains in-flight. Add focused RED coverage for the cancellation step/CLI scenario, then rerun the cancellation features and five clean full JVM runs. Resume at `spec/isaac/session/session_steps.clj:1302`, `spec/isaac/comm/comm_steps.clj:184`, and `features/session/cli.feature:354`.
