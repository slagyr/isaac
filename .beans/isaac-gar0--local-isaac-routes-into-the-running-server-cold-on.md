...
Done: Foundation `bean/isaac-gar0` @ `55b586b` adds raw pointer reading, `--local`, pre-config remote routing, failure/module diagnostics, routing steps, and activated scenarios. Proxy `bean/isaac-gar0` @ `02033b5` adds `remote use|off|status`, pointer preservation/private literal-token writes, exit 69/77 diagnostics, failure fixture steps, and activated scenarios. Proxy remote feature is green (29 examples/93 assertions); proxy specs are green (24/61). Foundation routing was green (8/20) before restoring root-spec isolation; root/args specs are now green (24/25).

Current state: RED Foundation routing after isolation restoration because real pointer fixtures are no longer visible through mem-fs; proxy full CI also has one unrelated slow integration assertion changed by improved auth wording. Next: repair the feature-only pointer bridge without global root state leakage at `spec/isaac/remote_routing_steps.clj:9` / `src/isaac/config/root.clj:66`, rerun Foundation routing, then align the slow auth assertion and run both CIs.
