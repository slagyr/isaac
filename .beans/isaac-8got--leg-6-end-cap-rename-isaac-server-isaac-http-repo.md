...
## CLI Proxy CI repair checkpoint (scrapper@isaac-work-1, 2026-09-14)

Done: reproduced Actions run 34791228232 (`isaac.tool.names` missing); advanced native/test Agent pins to landed `104b3c4`, Foundation pins to landed `82e3594`, CLI Server dependency/fixture to landed `ae0743a`; repaired empty-vector argv matching under current step-table semantics; and forced integration discovery to use the pinned CLI Server instead of the stale shared sibling. Commits through `965de91` are pushed to `bean/isaac-8got`. Focused `bb spec` is green 17/0/52 and `bb features` is green 11/0/40.

Current test state is red: `bb features-slow features/integration.feature:9` now reaches `/cli`, but remote subprocess startup fails inside CLI Server logging because `log/log*` receives `*file* = nil` on an HTTP-kit virtual thread; `isaac.logger/normalize-file-path` throws from `clojure.string/replace`. Exact resume point: `spec/isaac/cli_proxy/integration_steps.clj:166` (`remote-cli-ready`) — bind/wrap CLI Server dispatch with a non-nil file source for the feature harness, rerun slow scenarios at lines 9, 19, 30, and 40, then full `bb ci`. CI repair only; do not hail verify or perform landing coordination.
