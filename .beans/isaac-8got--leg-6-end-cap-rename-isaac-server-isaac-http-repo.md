...
## iMessage CI repair checkpoint (scrapper@isaac-work-1, 2026-09-14)

Done: reproduced all five JVM integration failures from Actions run 34791226244. The test harness was still booting only the iMessage contribution, without the landed built-in HTTP component index or explicit runner module index. Updated `spec/isaac/server/imessage_app_spec.clj` to use the post-component-cutover harness shape: merged built-ins, explicit `:module-index`, fresh component/comm registries, activation resets, and isolated nexus. Focused JVM suite is green: 50 examples, 0 failures, 81 assertions.

Current full `ISAAC_TEST_TIMEOUT_MS=600000 bb ci` is red only in three feature assertions: `features/comm/imessage/intake_filtering.feature:22` cannot see the emitted drop log, and `features/comm/imessage/send.feature:15,28` leave pending delivery files. These failures reproduce with both Foundation ad0a97b and 82e3594, so the speculative dependency repins were reverted. Next: repair feature harness isolation in `spec/isaac/comm/imessage/imessage_steps.clj:43` (`default-imessage-setup`) and `:71` (`imessage-delivery-worker-ticks`), rerun focused features, then full `bb ci`.
