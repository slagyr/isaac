...
Done: reproduced Actions run 34791228232 (`isaac.tool.names` missing), advanced CLI Proxy's native/test Agent pins to landed Agent `104b3c4`, advanced CLI Server dependency and integration fixture to landed `ae0743a`, repaired empty-vector argv matching under the new Agent step-table semantics, and made integration discovery ignore stale pre-cutover `../isaac-cli-server` siblings. Commits through `d6c06d2` are pushed to `bean/isaac-8got`. Focused `bb spec` is green 17/0/52 and `bb features` is green 11/0/40.

Current `bb ci` is red only in `@slow` integration after eliminating the old/new berth collision: scenarios at `features/integration.feature:9,30,40` now fail because successful remote commands produce no expected stdout and ACP reaches EOF. Next: inspect captured client/server output and launcher behavior starting at `spec/isaac/cli_proxy/integration_steps.clj:89` (`-install-test-isaac-launcher!`) and `:160` (`remote-cli-ready`), then rerun `bb features-slow features/integration.feature:9` and full `bb ci`. CI repair only; do not hail verify or perform landing coordination.
