...
- [x] Declare :isaac.config/check in the foundation manifest.
- [x] New server ns src/isaac/config/checks.clj: move check-tools,
...
- [x] Make the generic engines they use public foundation fns
...
- [x] Loader invokes each contributed check with a ctx map
...
- [x] One green sub-step per check; server manifest contribution added as each
      moves.
- [x] Verify error-key parity against loader_spec golden outputs
      (berths/normalize-errors post-processes and errors sort by :key, so
      contributed-check ordering should be cosmetic).
- [x] Audit known-provider-ids / known-comm-ids / declared-module-api-ids for
