...
Done: Agent branch `bean/isaac-lrue` is clean and pushed at `3a1cbff`: marker clearing removes current, flat, and legacy paths; resume requeues carry deterministic `:resume/requeued-at`; focused acceptance is green; full specs are green (1597 examples, 0 failures, 3292 assertions). Hail branch is clean and pushed at `4d2f698`: recent resume requeues survive the stale sweep, genuine stale deliveries are removed, and removal posts attention; focused worker specs are green (30 examples, 0 failures, 85 assertions) and focused marker acceptance is green (3 examples, 0 failures, 20 assertions).

Next/red: required broad CI gates are not yet green. Agent `bb ci` times out in `bb features` after 180 seconds with multiple feature failures; isolate the first failure before changing product code. Hail upstream CI run `34699892295` is independently red because `origin/main` references nonexistent Agent SHA `86a9a92ce6e665526f48ce6804945f80a07c593a`; rebase the bean branch onto current `origin/main`, preserve the bean's valid Agent pin, then run every acceptance command. Resume at `isaac-hail-lrue/deps.edn:8` (Agent SHA), followed by Agent feature isolation from `isaac-agent-lrue/bb.edn:45` (`features` task).
